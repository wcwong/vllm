...
For CUDA Unified Memory advice on UVA CPU-offloaded model weights, use
`--offload-memory-advice cuda_um_hints` with `--offload-backend uva` and
`--cpu-offload-gb > 0`. See
